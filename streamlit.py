...
agents_prompts = {
    1: "Suppose you're the reporter providing patient's information to the nurse. List the patient's general information in bullet points, only the necessary information and would not give long reports, avoid introducing yourself",
    2: "Suppose you are the patient, What is your current blood pressure and any other relevant health details and current feelings? Any pain? Any other symptoms.You're facing with a nurse so avoid anything unrelated to your symptoms/feelings etc",
    3: "Suppose you are a doctor, you have read the patient's information and all the measurements, provide professional diagnosis and instrution in the doctor's manner, you shall report some placeholder response for the simulation."
}
agents_names = {
    1: "Agent 1 (Reporter)",
    2: "Agent 2 (Patient)",
...
# user_input is initialized and displayed using the st.text_input function
...
# append msgs to chat history with current agent tag
        ...
        response = query_ollama(agents_prompts[st.session_state['agent_stage']])
        st.session_state['messages'].append({
            'agent': st.session_state['agent_stage'],  # Associate the agent response with the current agent
            'sender': agents_names[st.session_state['agent_stage']],
            'text': response
        })
...
st.markdown(f"### Conversation with {agents_names[st.session_state['agent_stage']]}")
for message in st.session_state['messages']:
    if message['agent'] == st.session_state['agent_stage']:
        st.write(f"{message['sender']}: {message['text']}")

# Options after processing the current stage
if st.session_state['stage_processed'] and st.session_state['agent_stage'] <= 3:
    # Option to ask another question in the current stage
    if st.button(f"Ask another question to {agents_names[st.session_state['agent_stage']]}"):
        st.session_state['stage_processed'] = False  # Allow another question
    
    # Option to proceed to the next stage
    if st.session_state['agent_stage'] < 3 and st.button(f"Proceed to {agents_names[st.session_state['agent_stage'] + 1]}"):
...
if st.session_state['agent_stage'] == 4:
    st.success("Interaction complete! You have finished all stages.")

# Button to reset the conversation and start again
